...
- Index events within the last **12 months** (configurable).
...
| 6 | EAC Completed | `Session_3_Date` not null AND ≥ 30 days elapsed | #2 |
| 7 | Post-EAC VL Sample | `Sample_Date > Index_VL_Date` AND `> Session_1_Date` | #2 |
| 8 | **Post-EAC VL Result** | `Post_EAC_VL_Result_Date > Index_VL_Result_Date` | **#2** *(changed — see §4.1)* |
| 9a | Re-suppressed | Post-EAC VL < 1,000 | #8 |
| — | · Undetectable | Post-EAC VL < 50 | #8 |
| — | · LLV | 50 ≤ Post-EAC VL < 1,000 | #8 |
| 9b | Still Unsuppressed | Post-EAC VL ≥ 1,000 | #8 |
| 10 | **Switched to Second-Line** | `Current_Regimen_Line` = second-line | **#1** *(changed — see §4.1)* |
...
6. **Second-line definition**: from `Current_Regimen_Line`. Attribution to a cycle awaits a switch date.
